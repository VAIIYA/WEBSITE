...
import { getAllPostsEn, getCoverImage } from '@/lib/posts'

export const metadata: Metadata = {
  title: 'News',
  description: 'AI industry news and development updates from VAIIYA.',
}
    ...
    <main className="min-h-screen bg-white">
      <section className="relative py-24 px-4 sm:px-6 lg:px-8 overflow-hidden bg-dot-grid">
        <div className="max-w-7xl mx-auto relative z-10 text-center">
          ...
          <p className="text-sm text-gray-400 max-w-xl mx-auto mt-6">
            New articles are published in English going forward — our older Dutch archive is being translated over time.
          </p>
        </div>
          ...
          {posts.length === 0 ? (
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
              {posts.map((post) => {
                ...
                >
                  <a href={post.translated ? `/en/news/${post.slug}` : `/news/${post.slug}`} className="block">
                    <div className="aspect-[16/9] w-full overflow-hidden bg-metamask-gray-50 relative">
                      ...
                      />
                      <span className="absolute top-3 right-3 px-2.5 py-1 rounded-full bg-black/60 backdrop-blur-md text-white text-[10px] font-bold uppercase tracking-wider">
                        {post.translated ? '🇬🇧 English' : '🇳🇱 Dutch'}
                      </span>
                    </div>
                    <div className="p-6 pb-0">
                      ...
                      {post.excerpt && (
                        <p className="text-sm text-gray-600 leading-relaxed pb-6">{post.excerpt}</p>
                      )}
                    </div>
                  </a>
                </div>
