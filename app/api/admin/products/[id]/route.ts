...
import { deleteProduct, getProductById, isSlugTaken, updateProduct, type ShopProductInput } from '@/lib/shop'
...
export async function PUT(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (!existing) return NextResponse.json({ error: 'Product niet gevonden.' }, { status: 404 })
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    return NextResponse.json({ error: 'Er ging iets mis bij het verwijderen.' }, { status: 500 })
  }
}
